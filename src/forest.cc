...
void Forest::sampleCoalescences( Node *start_node, bool record_and_bias ) {
  ...
  for (TimeIntervalIterator ti(this, active_node(0)); ti.good(); ++ti) {
    ...
    if ( tmp_event_.isNoEvent() ) {
      if ( record_and_bias ) this->record_all_event(ti); // Record events within this interval
      ...
      if ( record_and_bias ) this->record_all_event(ti); // Record the coalescence event (after implementing it)
      this->coalescence_finished_ = true;
    }

    else if ( tmp_event_.isRecombination() ) {
      if ( record_and_bias ) this->record_all_event(ti); // Record the recombination event
      this->implementRecombination(tmp_event_, ti);
    }

    else if ( tmp_event_.isMigration() ) {
      if ( record_and_bias ) this->record_all_event(ti); // Record the migration event
